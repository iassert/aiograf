...
class DillStorage(BaseStorage):
    FILE_PATH: str = os.path.join(os.getcwd(), "storge", "dill_storge.dill")
    ...
    @staticmethod
    def init(_file_: str, *, file_name: str = "dill_storge"):
        storge_dir = os.path.abspath(_file_).replace(os.path.basename(_file_), "storge")
        DillStorage.FILE_PATH = os.path.join(storge_dir, file_name)
    ...
    def load_data(self):
        if not os.path.exists(DillStorage.FILE_PATH):
            return {}

        try:
            with open(DillStorage.FILE_PATH, 'rb') as file:
    ...
    async def save_data_async(self):
        async with self.lock:
            try:
                async with aiofiles.open(DillStorage.FILE_PATH, 'wb') as file:
